...
# choropleth data
states <- geojsonio::geojson_read("~/Desktop/ADS/proj 2/station_distribution.geojson", what = "sp")
...
function(input, output) {
  ...
  output$intro_pic <- renderImage({
    filename <- 
      (file.path('~/Desktop/ADS/proj 2/intropic.jpg'))
    list(src = filename)
  }, deleteFile = FALSE)
  
  output$intro_type <- renderImage({
    filename <- 
      (file.path('~/Desktop/ADS/proj 2/icons.jpg'))
    list(src = filename)
  }, deleteFile = FALSE)
